...
def ExtractTrace():
    ...
    print ('Raw Traces: ' + str(len(lines_with_bracket)))
    print ('Unique Traces: ' + str(len(unique_lines_with_bracket)))
    # print (unique_lines_with_bracket[1][0])
    time.sleep(5)
...
if __name__ == "__main__":
    ...
    for i in range(len(deviant_trace)):  # for each deviant trace, load each FSM and query, then log the result
        ...
        for key in Result:
            ...
            if "Iphone" in output_dict["2_devices"]:
                correct_behavior = key

            trace_dict["2_outputs"].append(output_dict)

        vote_result = Majorityvote(Result, device_list)
        trace_dict["3_vote_result"] = [vote_result[key] for key in key_list]

        max_idx = get_max_idx(trace_dict["3_vote_result"])

        trace_dict["4_voted_behavior"] = str(key_list[max_idx])
        trace_dict["5_correct_behavior"] = correct_behavior

        trace_dict["6_underspecified"] = False
        trace_dict["7_unsure"] = False
        trace_dict["8_checked"] = False
    ...
    #   Write2File(Result, vote_reult)

    # Query = ['auth_request_plain_text', 'enable_s1', 'auth_request_plain_text', 'nas_sm_cmd', 'deregistration_req_protected']
    # FSMpath = './FSM/Redmagic.dot'
    # response = LQFSM(Query,FSMpath)
    # print(response)
