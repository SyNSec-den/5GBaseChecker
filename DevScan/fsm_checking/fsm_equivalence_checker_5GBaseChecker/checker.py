...
from graph import get_all_paths
...
def compute_diff(nuxmfilename, commandfilename, outputfilename, input_condition = None, fsm1_action=None, fsm2_action=None, fsm1_inFile=None, fsm2_inFile=None):
    # if not (input_condition == "deregistration_req_protected" and fsm1_action == "null_action" and fsm2_action == "deregistration_accept"):
    #     return []
    ...
    while True:
        ...
        if input_condition == input_list[len(input_list)-1]:
            ...
            if not fsm1_real_output == fsm2_real_output:
                output_list.append(s)

            # temp_out_file.write("\n\nInput: " + str(ctx_inputs) + "\n")

            fsm1_all_paths, fsm1_all_input_seqs, fsm1_all_output_seqs = get_all_paths(fsm1_inFile, fsm1_states[-1])
            fsm2_all_paths, fsm2_all_input_seqs, fsm2_all_output_seqs = get_all_paths(fsm2_inFile, fsm2_states[-1])
            # temp_out_file.write("fsm1_states: " + str(fsm1_states) + "\n")
            # temp_out_file.write("fsm1_all_paths: " + str(fsm1_all_paths) + "\n")
            # temp_out_file.write("fsm1_all_input_seqs: " + str(fsm1_all_input_seqs) + "\n")
            # temp_out_file.write("fsm1_all_output_seqs: " + str(fsm1_all_output_seqs) + "\n")
            #
            # temp_out_file.write("fsm2_states: " + str(fsm2_states) + "\n")
            # temp_out_file.write("fsm2_all_paths: " + str(fsm2_all_paths) + "\n")
            # temp_out_file.write("fsm2_all_input_seqs: " + str(fsm2_all_input_seqs) + "\n")
            # temp_out_file.write("fsm2_all_output_seqs: " + str(fsm2_all_output_seqs) + "\n")

            new_input_seqs = fsm1_all_input_seqs
            new_input_seqs.extend(fsm2_all_input_seqs)

            for input_seq in new_input_seqs:
                input_seq.append(ctx_inputs.split()[-1])
                input_seq_str = " ".join(input_seq)

                temp_fsm1_outputs, temp_fsm1_states = LQFSM(input_seq, fsm1_inFile)
                temp_fsm1_real_output = " ".join(temp_fsm1_outputs)
                temp_fsm2_outputs, temp_fsm2_states = LQFSM(input_seq, fsm2_inFile)
                temp_fsm2_real_output = " ".join(temp_fsm2_outputs)

                # this is to avoid false deviation paths
                if temp_fsm1_real_output == temp_fsm2_real_output:
                    continue

                s = "NEW PATH: [" + input_seq_str + " / " + temp_fsm1_real_output + "] [" + input_seq_str + " / " + temp_fsm2_real_output + "]"
                if not temp_fsm1_real_output == temp_fsm2_real_output:
                    output_list.append(s)
        ...
        input_symbol = ctx_inputs.split()[-1]
        edge_str = "{} -> {} [label=\"{}".format(src_state, dst_state, input_symbol)

        for line in lines:
            if edge_str in line and input_condition in line and ue2_ctx_output in line and flag == 0:
                flag = 1
