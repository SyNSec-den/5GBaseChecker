...
def dump_property(file, property ):
    file.write('\n\n' + property + '\n')
...
def do_iterative_checker(outNuXmv, commandFile, outDiff, fsm1_inFile , fsm2_inFile):
    ...
    for i in range (len(all_incoming_messages)-1, -1, -1):
        ...
        for j in list(perm):
            ...
            print("BEFORE COMPUTE DIFF :", all_incoming_messages[i], j[0], j[1])

            out = checker.compute_diff( outNuXmv, commandFile, outDiff, all_incoming_messages[i], j[0], j[1], fsm1_inFile, fsm2_inFile)
            if out == []:
                continue

            # # removing duplicates from global set
            # out_temp = []
            # for item in out:
            #     item_clean = item.replace("NEW PATH:", "").strip()
            #     if item_clean not in all_unique_path_set:
            #         out_temp.append(item)
            #         all_unique_path_set.add(item)
            #         all_unique_path_set.add(item_clean)
            # out = out_temp
...
def main(fsm1_inFile, fsm2_inFile, outNuXmv, commandFile, outDiff):
    ...
    for in_msg in fsm2.incoming_messages:
        ...
        if in_msg not in incoming_messages and in_msg not in 'null_action':
            incoming_messages.append(in_msg)
    print ("de iterative checker!!")
    do_iterative_checker(outNuXmv, commandFile, outDiff, fsm1_inFile, fsm2_inFile)
    return


if __name__ == "__main__":
    print("iterative_checker : start")

    parser = argparse.ArgumentParser(description="""DOT to SMV translator.""")

    parser.add_argument('-lts1', dest='lts1_dot_file', default="FSM/FSM1-5G.dot", help="UE NAS LTS")
    parser.add_argument('-lts2', dest='lts2_dot_file', default="FSM/FSM2-5G.dot", help="AMF NAS dot file to read")
    ...
    print("iterative_checker :", args.lts1_dot_file, args.lts2_dot_file, args.outNuXmv, args.commandFile, args.outDiff)
    ...
    # removing duplicates
    input_file = open(args.outDiff+"_final", "r")
    input_lines = input_file.readlines()
    input_file.close()

    output_lines = []
    all_unique_path_set = set()

    # adding all old paths
    for line in input_lines:
        if "NEW PATH:" not in line:
            all_unique_path_set.add(line.strip())

    # Checking new paths and keeping unique ones
    for line in input_lines:
        if "NEW PATH:" in line:
            changed_line = line.replace("NEW PATH:", "").strip()
            if changed_line in all_unique_path_set:
                continue
            else:
                output_lines.append(line)
                all_unique_path_set.add(changed_line)
        else:
            output_lines.append(line)

    output_file = open(args.outDiff+"_final", "w")
    ...
    with open(file_path, 'a') as file:
        file.write("{}, {}\n".format(args.outDiff, (time.time() - start_time)))
