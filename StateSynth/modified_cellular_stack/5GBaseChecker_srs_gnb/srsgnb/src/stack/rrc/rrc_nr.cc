...
{
  srsran::console("Fuzzing test. rrc_nr.cc handle_ul_ccch function called.\n");
  // Parse UL-CCCH
...
void rrc_nr::handle_ul_dcch(uint16_t rnti, uint32_t lcid, srsran::const_byte_span pdu)
{
  ...
  switch (ul_dcch_msg.msg.c1().type().value) {
    ...
    case ul_dcch_msg_type_c::c1_c_::types_opts::security_mode_fail:
      printf("types_opts::security_mode_fail\n");
      u.handle_security_mode_fail(); //Seems not functioning
      break;
    case ul_dcch_msg_type_c::c1_c_::types_opts::counter_check_resp:
      printf("types_opts::counter_check_resp\n");
      u.handle_countercheck_response(); //Seems not functioning
      break;
    case ul_dcch_msg_type_c::c1_c_::types_opts::rrc_recfg_complete:
      srsran::console("handle_dl_dcch rrc_recfg_complete\n");
...
int rrc_nr::start_security_mode_procedure(uint16_t rnti, srsran::unique_byte_buffer_t nas_pdu)
{
  ...
  if (nas_pdu != nullptr) {
      ...
      user_it->second->nas_pdu_queue.push_back(std::move(nas_pdu));
  }

  //user_it->second->send_security_mode_command(std::move(nas_pdu));//DIKEUE. Prevent enb send security_mode_command.
  //Todo: Further check. Why here nas_psu exist? what's the content?
  return SRSRAN_SUCCESS;
}
                                 ...
                                 uint32_t                five_qi)
{
  ...
  //users[rnti]->send_rrc_reconfiguration();//DIKEUE. prevent enb send rrc_reconfiguration.
  return SRSRAN_SUCCESS;
}
...
void rrc_nr::release_user(uint16_t rnti)
{
  ...
  users[rnti]->send_rrc_release(); //DIKEUE. prevent gnb send rrc_release.
...
void* rrc_nr::get_rrc_ue()
{
  if(users.empty()){
    srsran::console("get_rrc_ue user map empty!");
    return nullptr;
  }
  auto iter = users.begin();
  return iter->second.get(); //todo: not sure the user table will only contain 1 user all the time or we need to get the last one.
}

unsigned short rrc_nr::get_rrc_ue_rnti()
{
  if(users.empty()){
    srsran::console("get_rrc_ue user map empty!");
    return -1;
  ...
  auto iter = users.begin();
  srsran::console("%lu users in the user map.\n",users.size());
  srsran::console("returned rrc_nr_ue rnti!");
  return iter->first; //todo: not sure the user table will only contain 1 user all the time or we need to get the last one.
}
