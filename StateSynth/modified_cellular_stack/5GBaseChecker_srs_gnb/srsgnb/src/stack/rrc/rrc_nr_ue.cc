...
asn1::rrc_nr::dl_dcch_msg_s rrc_sm_cmd_replay_buf;
uint8_t                    rrc_transaction_id_backup = 0; //for rrc_sm_cmd_replay
uint8_t                    rrc_reconf_transaction_id_backup = 0; //for rrc_sm_cmd_replay
...
rrc_nr::ue::ue(rrc_nr* parent_, uint16_t rnti_, uint32_t pcell_cc_idx, bool start_msg3_timer) :
  parent(parent_), logger(parent_->logger), rnti(rnti_), uecfg(), sec_ctx(parent_->cfg)
{
...
int rrc_nr::ue::send_dl_dcch_replay(srsran::nr_srb srb, const asn1::rrc_nr::dl_dcch_msg_s& dl_dcch_msg)
{
  // Allocate a new PDU buffer, pack the message and send to PDCP
  srsran::unique_byte_buffer_t pdu = parent->pack_into_pdu(dl_dcch_msg, __FUNCTION__);
  if (pdu == nullptr) {
  ...
  fmt::format_to(fmtbuf, "DL-DCCH.{}", dl_dcch_msg.msg.c1().type().to_string());
  log_rrc_message(srb, Tx, *pdu.get(), dl_dcch_msg, srsran::to_c_str(fmtbuf));
  parent->pdcp->write_sdu_replay(rnti, srsran::srb_to_lcid(srb), std::move(pdu));
...
void rrc_nr::ue::handle_rrc_setup_request(const asn1::rrc_nr::rrc_setup_request_s& msg)
{
  ...
  send_rrc_setup(); //DIKEUE. Prevent enb send rrc_setup.
  enodeb_statelearner::get_instance()->notify_rrc_connection();

//  uint8_t response[20] = "rrc_setup_request\n"; //  a little bit oversize.
//  uint8_t size = 20;
//  parent->ngap->notify_response(response,size);
  set_activity_timeout(UE_INACTIVITY_TIMEOUT);

  // Fuzzing. Do we need to send a response to Log_executor here?
}
...
void rrc_nr::ue::handle_rrc_reestablishment_request(const asn1::rrc_nr::rrc_reest_request_s& msg)
{
  ...
  send_connection_reest(old_ue->sec_ctx.get_ncc()); //Fuzzing. Do we need to comment out this line?
  ...
  uint8_t response[15] = "rrc_reest_req\n"; //  a little bit oversize.
  uint8_t size = 15;
  parent->ngap->notify_response(response,size);
}
...
{
  srsran::console("send rrc reestablish.");
    if (not sec_ctx.is_as_sec_cfg_valid()) {
        srsran::console("ctxt check: k_gnb is not exist! return null action.");
  ...
  //since we are sending protected msg here.
  update_as_security(srb_to_lcid(srsran::nr_srb::srb1), true, true);
...
{
  srsran::console("rrc_nr_ue.CC LINE 994 send_rrc_setup()\n");
  const uint8_t max_wait_time_secs = 16;
  ...
  if (send_dl_ccch(msg) != SRSRAN_SUCCESS) { //This line is real send
    send_rrc_reject(max_wait_time_secs);
  }
}
...
void rrc_nr::ue::handle_rrc_setup_complete(const asn1::rrc_nr::rrc_setup_complete_s& msg)
{
  ...
  parent->ngap->initial_ue(
    ...
    enodeb_statelearner::get_instance()->RACH_Counter(); //RACH_Counter++

  //Fuzzing
//  uint8_t response[22] = "rrc_setup_complete\n"; //  a little bit oversize.
//  uint8_t size = 22;
//  parent->ngap->notify_response(response,size);
}
...
{
  srsran::console("rrc_nr::ue::send_security_mode_command\n");
  // context check
    if (not sec_ctx.is_as_sec_cfg_valid()) {
        srsran::console("ctxt check: k_gnb is not exist! return null action.");
        uint8_t response[13] = "null_action\n"; //  a little bit oversize.
  ...
  update_as_security_int(srb_to_lcid(srsran::nr_srb::srb1), true, false);

  if (nas_pdu != nullptr) {
    srsran::console("rrc_nr::ue::send_security_mode_command nas_pdu != nullptr \n");
  ...
  rrc_transaction_id_backup = dl_dcch_msg.msg.set_c1().set_security_mode_cmd().rrc_transaction_id; //for rrc_sm_cmd_replay.
  ...
  rrc_sm_cmd_replay = 1; //set replay msg flag to 1. Means we can send replay msg now.

//  rrc_sm_cmd_replay_buf.msg.set_c1().set_security_mode_cmd().rrc_transaction_id = (uint8_t)((rrc_transaction_id_backup++) % 4);
//  security_mode_cmd_ies_s& iies = rrc_sm_cmd_replay_buf.msg.c1().security_mode_cmd().crit_exts.set_security_mode_cmd();
//  iies.security_cfg_smc.security_algorithm_cfg.integrity_prot_algorithm_present = true;
//  iies.security_cfg_smc.security_algorithm_cfg                                  = sec_ctx.get_security_algorithm_cfg();

  if (send_dl_dcch(srsran::nr_srb::srb1, dl_dcch_msg) != SRSRAN_SUCCESS) {
    rrc_sm_cmd_replay = 0; //For robustness.
    srsran::console("rrc_nr_ue 1173 send_security_mode_command wrong!\n");
    //parent->ngap->user_release_request(rnti, asn1::ngap::cause_radio_network_opts::radio_res_not_available);
  }
}

void rrc_nr::ue::send_security_mode_command_null_sec(srsran::unique_byte_buffer_t nas_pdu)
{
  srsran::console("rrc_nr::ue::send_security_mode_command_ns\n");
  // apply selected security config and enable integrity on SRB1 before generating security mode command
  ...
  if (nas_pdu != nullptr) {
    srsran::console("rrc_nr::ue::send_security_mode_command_ns nas_pdu != nullptr \n");
    nas_pdu_queue.push_back(std::move(nas_pdu));
  }
  ...
  ies.security_cfg_smc.security_algorithm_cfg.integrity_prot_algorithm_present = false;
  ies.security_cfg_smc.security_algorithm_cfg                                  = sec_ctx.get_security_algorithm_cfg();

  ciphering_algorithm_e cipher_alg;
  ies.security_cfg_smc.security_algorithm_cfg.ciphering_algorithm = cipher_alg.nea0;

  integrity_prot_algorithm_e int_alg;
  ies.security_cfg_smc.security_algorithm_cfg.integrity_prot_algorithm = int_alg.nia0;

  if (send_dl_dcch(srsran::nr_srb::srb1, dl_dcch_msg) != SRSRAN_SUCCESS) {
    srsran::console("rrc_nr_ue 1202 send_security_mode_command_null_sec wrong!\n");
    //parent->ngap->user_release_request(rnti, asn1::ngap::cause_radio_network_opts::radio_res_not_available);
  }
}

void rrc_nr::ue::send_security_mode_command_replay(srsran::unique_byte_buffer_t nas_pdu){
  srsran::console("rrc_nr::ue::send_security_mode_command_replay\n");
  // apply selected security config and enable integrity on SRB1 before generating security mode command

    if(!rrc_sm_cmd_replay){
        srsran::console("ctxt check: rrc_sm_cmd_replay can not send! return null action.");
        uint8_t response[13] = "null_action\n"; //  a little bit oversize.
        uint8_t size = 13;
        parent->ngap->notify_response(response,size);
        return;
  ...
  asn1::rrc_nr::dl_dcch_msg_s dl_dcch_msg;
  dl_dcch_msg.msg.set_c1().set_security_mode_cmd().rrc_transaction_id = (uint8_t)((rrc_transaction_id_backup) % 4); //to keep transaction_id the same as the last rrc_sm_cmd.
  security_mode_cmd_ies_s& ies = dl_dcch_msg.msg.c1().security_mode_cmd().crit_exts.set_security_mode_cmd();

  ies.security_cfg_smc.security_algorithm_cfg.integrity_prot_algorithm_present = true;
  ies.security_cfg_smc.security_algorithm_cfg                                  = sec_ctx.get_security_algorithm_cfg();

  if (send_dl_dcch_replay(srsran::nr_srb::srb1, dl_dcch_msg) != SRSRAN_SUCCESS) {
    parent->ngap->user_release_request(rnti, asn1::ngap::cause_radio_network_opts::radio_res_not_available);
  }
}

void rrc_nr::ue::send_security_mode_command_protected(srsran::unique_byte_buffer_t nas_pdu)
{
    srsran::console("rrc_nr::ue::send_security_mode_command_protected\n");
    if (not sec_ctx.is_as_sec_cfg_valid()) {
        srsran::console("ctxt check: k_gnb is not exist! return null action.");
        uint8_t response[13] = "null_action\n"; //  a little bit oversize.
        uint8_t size = 13;
    ...
    // apply selected security config and enable integrity on SRB1 before generating security mode command
    update_as_security(srb_to_lcid(srsran::nr_srb::srb1), true, true); //Set both as true to send security_mode_command_protected
...
void rrc_nr::ue::send_security_mode_command_plain_text(srsran::unique_byte_buffer_t nas_pdu)
{
    srsran::console("rrc_nr::ue::send_security_mode_command_plain_text\n");
    // apply selected security config and enable integrity on SRB1 before generating security mode command
    update_as_security_plain_text(srb_to_lcid(srsran::nr_srb::srb1), false, false); //Set both as false to send send_security_mode_command_plain_text
...
int rrc_nr::ue::reset_security(uint32_t lcid) {
    sec_ctx.sec_cfg = {};
    sec_ctx.reset_k_gnb_present_ncc();
    parent->pdcp->config_security(rnti,lcid,{});
    rrc_sm_cmd_replay = 0;
    rrc_reconf_replay = 0;
    return SRSRAN_SUCCESS;
}

int rrc_nr::ue::update_as_security_plain_text(uint32_t lcid, bool enable_integrity = false, bool enable_ciphering = false)
{
    if (not sec_ctx.is_as_sec_cfg_valid()) {
        parent->logger.error("Invalid AS security configuration. Skipping configuration for lcid=%d", lcid);
        return SRSRAN_SUCCESS;
    ...
    pdcp_cnfg.k_up_int                        = tmp_cnfg.k_nr_up_int;
    pdcp_cnfg.k_up_enc                        = tmp_cnfg.k_nr_up_enc;
    pdcp_cnfg.integ_algo                      = srsran::INTEGRITY_ALGORITHM_ID_EIA0;
    pdcp_cnfg.cipher_algo                     = srsran::CIPHERING_ALGORITHM_ID_EEA0;
...
int rrc_nr::ue::update_as_security_int(uint32_t lcid, bool enable_integrity = true, bool enable_ciphering = false)
{
    if (not sec_ctx.is_as_sec_cfg_valid()) {
        parent->logger.error("Invalid AS security configuration. Skipping configuration for lcid=%d", lcid);
        //return SRSRAN_ERROR;
...
void rrc_nr::ue::handle_security_mode_fail()           // mark!!!
{
    uint8_t response[18] = "rrc_sm_failure\n"; //  a little bit oversize.
    uint8_t size         = 18;
    parent->ngap->notify_response(response,18);
}

void rrc_nr::ue::handle_countercheck_response()
{
    uint8_t response[30] = "rrc_countercheck_response\n"; //  a little bit oversize.
    uint8_t size         = 30;
    parent->ngap->notify_response(response,size);
}
...
void rrc_nr::ue::handle_security_mode_complete(const asn1::rrc_nr::security_mode_complete_s& msg)
{
  ...
  //send_ue_capability_enquiry(); //DIKEUE. prevent enb send ue_capability_enquiry.

  uint8_t response[18] = "rrc_sm_complete\n"; //  a little bit oversize.
  uint8_t size = 18;
  parent->ngap->notify_response(response,size);
}
...
{
  srsran::console("send_rrc_reconfiguration\n");
    if (not sec_ctx.is_as_sec_cfg_valid()) {
        srsran::console("ctxt check: k_gnb is not exist! return null action.");
        uint8_t response[13] = "null_action\n"; //  a little bit oversize.
        uint8_t size = 13;
  ...
  rrc_reconf_transaction_id_backup = dl_dcch_msg.msg.set_c1().set_rrc_recfg().rrc_transaction_id; //for rrc_reconf_replay
  ...
  if (nas_pdu_queue.size() > 0) {
    srsran::console("NAS registration accept already in queue!");
  ...
  rrc_reconf_replay = 1; //replay msg flag.

  update_as_security(srb_to_lcid(srsran::nr_srb::srb1), true, true);

  if (send_dl_dcch(srsran::nr_srb::srb1, dl_dcch_msg) != SRSRAN_SUCCESS) {
    srsran::console("rrc_reconf failed to sent!");
    parent->ngap->user_release_request(rnti, asn1::ngap::cause_radio_network_opts::radio_res_not_available);
    rrc_reconf_replay = 0;
  }
}

void rrc_nr::ue::send_rrc_reconfiguration_replay()
{
    srsran::console("send_rrc_reconfiguration_replay\n");
    if(!rrc_reconf_replay){
        srsran::console("ctxt check: rrc_reconf_replay can not send!");
        uint8_t response[13] = "null_action\n"; //  a little bit oversize.
        uint8_t size = 13;
        parent->ngap->notify_response(response,size);
        return;
    ...
    dl_dcch_msg_s dl_dcch_msg;
    dl_dcch_msg.msg.set_c1().set_rrc_recfg().rrc_transaction_id = (uint8_t)((rrc_reconf_transaction_id_backup) % 4); //for replay.
    rrc_recfg_ies_s& ies = dl_dcch_msg.msg.c1().rrc_recfg().crit_exts.set_rrc_recfg();
...
void rrc_nr::ue::send_rrc_reconfiguration_plain_text()
{
    srsran::console("send_rrc_reconfiguration_plain_text\n");
    dl_dcch_msg_s dl_dcch_msg;
    dl_dcch_msg.msg.set_c1().set_rrc_recfg().rrc_transaction_id = (uint8_t)((transaction_id++) % 4);
    rrc_recfg_ies_s& ies = dl_dcch_msg.msg.c1().rrc_recfg().crit_exts.set_rrc_recfg();
    ...
    if (ies.radio_bearer_cfg_present) {
        ...
        // this is done after updating the RLC bearers,
        // so the PDCP can query the RLC mode
        update_pdcp_bearers_plain_text(ies.radio_bearer_cfg, master_cell_group);
    }
    ...
    //since we are sending plain msg here.
    update_as_security_plain_text(srb_to_lcid(srsran::nr_srb::srb1), false, false);
    if (send_dl_dcch(srsran::nr_srb::srb1, dl_dcch_msg) != SRSRAN_SUCCESS) {
        parent->ngap->user_release_request(rnti, asn1::ngap::cause_radio_network_opts::radio_res_not_available);
    }
}

int rrc_nr::ue::send_rrc_countercheck_plain_text()
{
    srsran::console("send_rrc_countercheck_plain_text");
    dl_dcch_msg_s dl_dcch_msg;
    ...
    count_msb_info.drb_id = 1;
    count_msb_info.count_msb_ul = 1;
    count_msb_info.count_msb_dl = 1;

    ies.drb_count_msb_info_list.push_back(count_msb_info);

    update_as_security_plain_text(srb_to_lcid(srsran::nr_srb::srb1), true, true);
    send_dl_dcch(srsran::nr_srb::srb1, dl_dcch_msg);
    return SRSRAN_SUCCESS;
}

int rrc_nr::ue::send_rrc_countercheck_protected()
{
    srsran::console("send_rrc_countercheck_protected");
    ...
    ies.drb_count_msb_info_list.push_back(count_msb_info);
    update_as_security_int(srb_to_lcid(srsran::nr_srb::srb1), true, true);
    send_dl_dcch(srsran::nr_srb::srb1, dl_dcch_msg);
    return SRSRAN_SUCCESS;

}

int rrc_nr::ue::send_rrc_ueinfo_req_plain_text()
{
    uint8_t response[13] = "null_action\n";
    uint8_t size = 13;
    parent->ngap->notify_response(response,size);
...
int rrc_nr::ue::send_rrc_ueinfo_req_protected()
{
    uint8_t response[13] = "null_action\n";
    uint8_t size = 13;
    parent->ngap->notify_response(response,size);
...
{
  srsran::console("send_ue_capability_enquiry");
    if (not sec_ctx.is_as_sec_cfg_valid()) {
        srsran::console("ctxt check: k_gnb is not exist! return null action.");
        uint8_t response[13] = "null_action\n"; //  a little bit oversize.
        uint8_t size = 13;
...
int rrc_nr::ue::send_ue_capability_enquiry_plaintext()
{
    srsran::console("send_ue_capability_enquiry_plaintext");
...
void rrc_nr::ue::handle_ue_capability_information(const asn1::rrc_nr::ue_cap_info_s& msg)
{
  ...
  if (not nas_pdu_queue.empty()) {
    //send_rrc_reconfiguration();// Do we need to comment it out?
  }

  uint8_t response[18] = "rrc_ue_cap_info\n"; //  a little bit oversize.
  uint8_t size = 18;
  parent->ngap->notify_response(response,size);
...
{
  srsran::console("rrc_nr::ue::handle_rrc_reconfiguration_complete called\n");
  update_mac(next_cell_group_cfg, true);
  ...
  uint8_t response[22] = "rrc_reconf_complete\n"; //  a little bit oversize.
  uint8_t size = 22;
  parent->ngap->notify_response(response,size);
...
void rrc_nr::ue::handle_rrc_reestablishment_complete(const asn1::rrc_nr::rrc_reest_complete_s& msg)
{
  ...
  send_rrc_reconfiguration(); //Fuzzing. Do we need to comment out this line?

  uint8_t response[21] = "rrc_reest_complete\n"; //  a little bit oversize.
  uint8_t size = 21;
  parent->ngap->notify_response(response,size);
}
...
void rrc_nr::ue::send_rrc_release()
{
  ...
  parent->task_sched.defer_callback(release_delay, [this]() { parent->rem_user(rnti); });
    ...
    void rrc_nr::ue::sendd_rrc_release() //for state learner
    {
        srsran::console("rrc_nr::ue::send_rrc_release called\n");
        static const uint32_t release_delay = 60; // Taken from TS 38.331, 5.3.8.3
...
int rrc_nr::ue::update_pdcp_bearers_plain_text(const asn1::rrc_nr::radio_bearer_cfg_s& radio_bearer_diff,
                                    const asn1::rrc_nr::cell_group_cfg_s&   cell_group_diff)
{
    // release DRBs
    // TODO
    ...
    for (const srb_to_add_mod_s& srb : radio_bearer_diff.srb_to_add_mod_list) {
       ...
       // if (sec_ctx.is_as_sec_cfg_valid()) {
            update_as_security_plain_text(rlc_bearer->lc_ch_id); //set as plain_text. For rrc_reconf_plain_text.
       // }
    }
    ...
    for (const drb_to_add_mod_s& drb : radio_bearer_diff.drb_to_add_mod_list) {
        ...
        //if (sec_ctx.is_as_sec_cfg_valid()) {
        update_as_security_plain_text(rlc_bearer->lc_ch_id);
//        update_as_security(rlc_bearer->lc_ch_id, false, false); //set as plain_text. For rrc_reconf_plain_text.
        // }
    }
