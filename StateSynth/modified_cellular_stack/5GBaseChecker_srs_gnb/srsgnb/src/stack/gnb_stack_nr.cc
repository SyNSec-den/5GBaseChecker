                       ...
                       x2_interface*           x2_)
{
  args = args_;
  phy  = phy_;
  enodeb_statelearner = enodeb_statelearner::get_instance();
  // setup logging
  ...
  if (ngap != nullptr) {
    ...
    ngap->init(args.ngap, &rrc, gtpu.get(),enodeb_statelearner);
  ...
  srsran::console("gnb initialized!\n");
  enodeb_statelearner->init(ngap.get());
