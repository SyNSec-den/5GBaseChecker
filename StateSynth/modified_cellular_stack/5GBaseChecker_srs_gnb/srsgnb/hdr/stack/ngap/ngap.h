...
class ngap final : public ngap_interface_rrc_nr, public ngap_interface_enodeb_statelearner
{
public:
  class ue;
  ...
  ~ngap();
  int  init(const ngap_args_t& args_, rrc_interface_ngap_nr* rrc_, gtpu_interface_rrc* gtpu_, enodeb_statelearner_interface_ngap *enodeb_statelearner_);
  void stop();
  ...
  bool execute_command(uint8_t* msg, uint16_t len) override;

  //Fuzzing, state_learner
  //bool notify_response(uint8_t* msg, uint16_t len);
  bool notify_response(uint8_t* msg, uint16_t len) override;
...
private:
  ...
  ngap_args_t                 args = {};
  enodeb_statelearner_interface_ngap *enodeb_statelearner = nullptr;
  srslog::basic_logger&       logger;
