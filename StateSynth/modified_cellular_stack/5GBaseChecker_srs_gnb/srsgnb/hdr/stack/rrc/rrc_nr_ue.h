...
public:
  ...
  /// rrc_sm_cmd_replay and rrc_reconf_replay context check
  int rrc_sm_cmd_replay = 0;
  int rrc_reconf_replay = 0;
  ...
  void handle_security_mode_fail();
  ...
  void sendd_rrc_release(); //for state learner
  ...
  void handle_countercheck_response();
  ...
  void send_security_mode_command_replay(srsran::unique_byte_buffer_t nas_pdu);
  void send_security_mode_command_null_sec(srsran::unique_byte_buffer_t nas_pdu);
  void send_security_mode_command_protected(srsran::unique_byte_buffer_t nas_pdu);
  void send_security_mode_command_plain_text(srsran::unique_byte_buffer_t nas_pdu);
  void send_security_mode_command_plain_header(srsran::unique_byte_buffer_t nas_pdu);

  /* TS 38.331 - 5.3.5 RRC reconfiguration */
  void send_rrc_reconfiguration();
  void send_rrc_reconfiguration_replay();
  void send_rrc_reconfiguration_plain_text();
  /* TS 38.331 - 5.6.1 UE capability transfer */
  int send_ue_capability_enquiry();
  int send_ue_capability_enquiry_plaintext();


  int send_rrc_countercheck_plain_text();
  int send_rrc_countercheck_protected();
  int send_rrc_ueinfo_req_plain_text();
  int send_rrc_ueinfo_req_protected();
  int reset_security(uint32_t lcid);

  // Security helper
  srsgnb::nr_security_context sec_ctx; //To make rrc_reestablish message successful in execute command function.

    std::vector<srsran::unique_byte_buffer_t> nas_pdu_queue;
private: // Can we delete that????
  ...
  int send_dl_dcch_replay(srsran::nr_srb srb, const asn1::rrc_nr::dl_dcch_msg_s& dl_dcch_msg);

  /** TS 38.331 - 5.3.3 RRC connection establishment */
  void send_rrc_setup(); //Fuzzing. Why set it as private?
  ...
  int update_pdcp_bearers_plain_text(const asn1::rrc_nr::radio_bearer_cfg_s& radio_bearer_diff,
                        const asn1::rrc_nr::cell_group_cfg_s&   cell_group_diff);
  ...
  int update_as_security_plain_text(uint32_t lcid, bool enable_integrity, bool enable_ciphering);
  int update_as_security_int(uint32_t lcid, bool enable_integrity, bool enable_ciphering);
