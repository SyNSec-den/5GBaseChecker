...
public:
  ...
  void disable_integrity(srsran_direction_t direction = DIRECTION_NONE)
  {
    // if either DL or UL is already enabled, both are enabled
    if (integrity_direction == DIRECTION_TX && direction == DIRECTION_RX) {
      integrity_direction = DIRECTION_TXRX;
  ...
  void enable_encryption(srsran_direction_t direction = DIRECTION_TXRX)
  {
    ...
    if (encryption_direction == DIRECTION_TX && direction == DIRECTION_RX) {
      ...
      encryption_direction = direction; //set as None probably means disable encryption
      logger.info("pdcp_entity_base.h 113 encryption_direction = direction");
    }
    logger.info("Enabled encryption. LCID=%d, encryption=%s", lcid, srsran_direction_text[integrity_direction]);
  }

  void disable_encryption(srsran_direction_t direction = DIRECTION_NONE)
  {
    // if either DL or UL is already enabled, both are enabled
    if (encryption_direction == DIRECTION_TX && direction == DIRECTION_RX) {
      encryption_direction = DIRECTION_TXRX;
    ...
    logger.debug("Disable encryption. LCID=%d, encryption=%s", lcid, srsran_direction_text[integrity_direction]);
  }
  ...
  virtual void write_sdu_replay(unique_byte_buffer_t sdu, int sn = -1) = 0;
  // RLC interface
