...
void pdcp_entity_nr::write_sdu_replay(unique_byte_buffer_t sdu, int sn)
{
  printf("pdcp_entity_nr::write_sdu_replay");
  uint32_t tx_next_prev      = 0;
  tx_next_prev = tx_next;
//  tx_next = 2; //make replay packet's sequence number = 1
  tx_next--; //initial implementation
  // Log SDU
  logger.info(sdu->msg,
              sdu->N_bytes,
              "TX %s SDU (%dB), integrity=%s, encryption=%s",
