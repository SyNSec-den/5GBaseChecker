...
void pdcp::write_sdu_replay(uint16_t rnti, uint32_t lcid, srsran::unique_byte_buffer_t sdu, int pdcp_sn)
{
  if (users.count(rnti)) {
    if (rnti != SRSRAN_MRNTI) {
      // TODO: Handle PDCP SN coming from GTPU
      users[rnti].pdcp->write_sdu_replay(lcid, std::move(sdu), pdcp_sn);
    } else {
      users[rnti].pdcp->write_sdu_mch(lcid, std::move(sdu));
    }
  }
