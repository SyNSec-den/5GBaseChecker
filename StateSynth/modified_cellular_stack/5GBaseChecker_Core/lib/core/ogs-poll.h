...
typedef struct ogs_pollset_actions_s {
    void (*init)(ogs_pollset_t *pollset);               // init pollset
    void (*cleanup)(ogs_pollset_t *pollset);            // clear pollset

    int (*add)(ogs_poll_t *poll);                       // add one ele
    int (*remove)(ogs_poll_t *poll);                    // remove one ele

    int (*poll)(ogs_pollset_t *pollset, ogs_time_t timeout);      // ?
    int (*notify)(ogs_pollset_t *pollset);               // ?
} ogs_pollset_actions_t;

extern ogs_pollset_actions_t ogs_pollset_actions; // ogs_polllset_actions_t, defined in line 54
