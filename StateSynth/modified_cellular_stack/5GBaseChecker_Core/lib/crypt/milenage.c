    ...
    uint8_t *res, size_t *res_len)
{
	...
	*res_len = 8;
    ...
    ogs_warn("Calculating AUTN in milenage_generate");

	/* AUTN = (SQN ^ AK) || AMF || MAC */
//    if (SQN_OutofRange == 1){
//        ogs_warn("SQN_OutofRange == 1");
//        SQN_OutofRange = 0;
//
//        uint8_t *sqn_bkup = malloc(6);
//        memcpy(sqn_bkup,sqn, 6);
//
//        for (i = 0; i < 6; i++)
//            sqn_bkup[i] = 255; //not sure needs double check
//        for (i = 0; i < 6; i++)
//            autn[i] = sqn_bkup[i] ^ ak[i];
//    }else{
//        for (i = 0; i < 6; i++)
//            autn[i] = sqn[i] ^ ak[i]; //original implementation
//    }
