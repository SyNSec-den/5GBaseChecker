...
{
    ogs_info("handle_registration_request called");
    regis_count++; // Mainly for handle plaintext msg which made UE disconnect.
...
{
    ogs_info("handle_authentication_response called");
...
//        send_message("auth_failure_mac\n");
        ...
        return OGS_OK; // in case open5gs crash
        // return OGS_ERROR;
    }
    ...
    ogs_info("handle_authentication_response finished");
        ...
        ogs_nas_5gs_identity_response_t *identity_response)
{
    ...
    ogs_info("handle_identity_response_called");
    ...
    if (mobile_identity_header->type == OGS_NAS_5GS_MOBILE_IDENTITY_SUCI) {
        ...
        amf_ue_set_suci(amf_ue, mobile_identity);
        // amf_ue->suci="suci-0-001-01-0000-0-0-1234567895"; //srsue bug?
        ogs_info("[%s]    SUCI", amf_ue->suci);
    } else {
...
{
    ogs_info("handle_security_mode_complete called");
    ...
    if ((security_mode_complete->presencemask &
        OGS_NAS_5GS_SECURITY_MODE_COMPLETE_NAS_MESSAGE_CONTAINER_PRESENT)
            == 0) {
        ogs_warn("No NAS Message Container in Security mode complete message");
        return OGS_5GMM_CAUSE_REQUEST_ACCEPTED;
        ...
        ogs_nas_5gs_ul_nas_transport_t *ul_nas_transport)
{
    ...
    case OGS_NAS_PAYLOAD_CONTAINER_N1_SM_INFORMATION:
        ...
        if (gsm_header->message_type ==
            ...
            ogs_warn("OGS_NAS_5GS_PDU_SESSION_ESTABLISHMENT_REQUEST!!!");
            ...
            ogs_warn("OGS_NAS_5GS_PDU_SESSION_ESTABLISHMENT_REQUEST ELSE!!!");
            sess = amf_sess_find_by_psi(amf_ue, *pdu_session_id);
            if (!sess) {
