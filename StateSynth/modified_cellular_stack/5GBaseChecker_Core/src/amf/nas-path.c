...
ogs_pkbuf_t *smd_replay_buf = NULL;
ogs_pkbuf_t *config_update_cmd_replay_buf = NULL;
bool smd_replay_stored = false;
bool config_update_cmd_replay_stored = false;
...
int nas_5gs_send_to_gnb(amf_ue_t *amf_ue, ogs_pkbuf_t *pkbuf)
{
    ...
    ogs_info("enter nas_5gs_send_to_gnb");
    ogs_warn("pkbuf = %p", pkbuf);
...
int nas_5gs_send_to_downlink_nas_transport(amf_ue_t *amf_ue, ogs_pkbuf_t *pkbuf)
{
    ...
    ogs_info("enter nas_5gs_send_to_downlink_nas_transport");
    ogs_warn("ngapbuf = %p", ngapbuf);
...
int nas_5gs_send_registration_accept_dlnas(amf_ue_t *amf_ue) {
    int rv;
    ogs_pkbuf_t *gmmbuf = NULL;
    ogs_assert(amf_ue);
    gmmbuf = gmm_build_registration_accept(amf_ue);
...
int nas_5gs_send_registration_accept_plain_text_dlnas(amf_ue_t *amf_ue) {
    int rv;
    ogs_pkbuf_t *gmmbuf = NULL;
    ogs_assert(amf_ue);
    gmmbuf = gmm_build_registration_accept_plain_text(amf_ue);
    rv = nas_5gs_send_to_downlink_nas_transport(amf_ue, gmmbuf);
    return rv;
}

int nas_5gs_send_registration_accept_h4_plain_text_dlnas(amf_ue_t *amf_ue) {
    int rv;
    ogs_pkbuf_t *gmmbuf = NULL;
    ogs_assert(amf_ue);
    gmmbuf = gmm_build_registration_accept_h4_plain_text(amf_ue);
    rv = nas_5gs_send_to_downlink_nas_transport(amf_ue, gmmbuf);
    return rv;
}

int nas_5gs_send_registration_accept_plain_text(amf_ue_t *amf_ue)
{
    int rv;
    bool transfer_needed = false;
    ...
    if (amf_ue->next.m_tmsi) {
        ogs_info("amf_ue->next.m_tmsi passed.");
        if (amf_ue->t3550.pkbuf) {
            gmmbuf = amf_ue->t3550.pkbuf;
            ogs_expect_or_return_val(gmmbuf, OGS_ERROR);
        } else {
...
int nas_5gs_send_de_registration_request_plain_text(amf_ue_t *amf_ue)
{
    int rv;
    ...
    ogs_debug("[%s] De-registration request plaintext", amf_ue->supi);

    if (amf_ue->t3522.pkbuf) {
//        gmmbuf = amf_ue->t3522.pkbuf;
        gmmbuf = gmm_build_de_registration_request_plain_text(amf_ue);
        ogs_expect_or_return_val(gmmbuf, OGS_ERROR);
    } else {
        gmmbuf = gmm_build_de_registration_request_plain_text(amf_ue);
        ogs_expect_or_return_val(gmmbuf, OGS_ERROR);
...
//        ogs_expect_or_return_val(gmmbuf, OGS_ERROR);
//    } else {
        gmmbuf = gmm_build_security_mode_command(amf_ue); // build smd
        // ogs_expect_or_return_val(gmmbuf, OGS_ERROR);
        if (gmmbuf==NULL) {
            return -5;
            // 0: success,  -1 ~ -4 used
...
//    }
    if(!smd_replay_stored) {
        smd_replay_stored = true;
        smd_replay_buf = ogs_pkbuf_copy(gmmbuf);
    }
    amf_ue->t3560.pkbuf = ogs_pkbuf_copy(gmmbuf);
    ogs_expect_or_return_val(amf_ue->t3560.pkbuf, OGS_ERROR);
    ogs_timer_start(amf_ue->t3560.timer,
...
int nas_5gs_send_security_mode_command_replay(amf_ue_t *amf_ue)
{
    int rv;

    ogs_pkbuf_t *smd_replay_buf_backup = NULL;
    smd_replay_buf_backup = ogs_pkbuf_copy(smd_replay_buf);
    rv = nas_5gs_send_to_downlink_nas_transport(amf_ue, smd_replay_buf_backup);
...
int nas_5gs_send_security_mode_command_protected(amf_ue_t *amf_ue)
{
    int rv;
    ogs_pkbuf_t *gmmbuf = NULL;

    ogs_assert(amf_ue);
    ogs_info("smd_protected called 548");

    ogs_debug("[%s] Security mode command", amf_ue->supi);

    if (amf_ue->t3560.pkbuf) {
        gmmbuf = amf_ue->t3560.pkbuf;
        gmmbuf = gmm_build_security_mode_command_protected(amf_ue); 
        if (gmmbuf==NULL) {
            return -5;
            // 0: success,  -1 ~ -4 used
        }
...
int nas_5gs_send_security_mode_command_null_security(amf_ue_t *amf_ue)
{
    int rv;
    ogs_pkbuf_t *gmmbuf = NULL;
    ...
    if (amf_ue->t3560.pkbuf) {
        gmmbuf = amf_ue->t3560.pkbuf;
        gmmbuf = gmm_build_security_mode_command_null_security(amf_ue); // to debug
        if (gmmbuf==NULL) {
            return -5;
            // 0: success,  -1 ~ -4 used
        }
        ...
        gmmbuf = gmm_build_security_mode_command_null_security(amf_ue);
        if (gmmbuf==NULL) {
            return -5;
            // 0: success,  -1 ~ -4 used
        }
...
int nas_5gs_send_security_mode_command_null_security_int(amf_ue_t *amf_ue)
{
    int rv;
    ogs_pkbuf_t *gmmbuf = NULL;
    ...
    if (amf_ue->t3560.pkbuf) {
        gmmbuf = amf_ue->t3560.pkbuf;
        gmmbuf = gmm_build_security_mode_command_null_security_int(amf_ue); // to debug
        if (gmmbuf==NULL) {
            return -5;
            // 0: success,  -1 ~ -4 used
        }
        ...
        gmmbuf = gmm_build_security_mode_command_null_security_int(amf_ue);
        if (gmmbuf==NULL) {
            return -5;
            // 0: success,  -1 ~ -4 used
        }
        ...
        amf_ue_t *amf_ue, gmm_configuration_update_command_param_t *param)
{
    ...
    ogs_warn("[%s] Configuration update command with ack", amf_ue->supi);

    if (amf_ue->t3555.pkbuf) {
        gmmbuf = amf_ue->t3555.pkbuf;
        ogs_expect_or_return_val(gmmbuf, OGS_ERROR);
        gmmbuf = gmm_build_configuration_update_command(amf_ue, param); // to debug
    ...
    if(!config_update_cmd_replay_stored){
        config_update_cmd_replay_stored = true;
        config_update_cmd_replay_buf = ogs_pkbuf_copy(gmmbuf); //for replay version.
    }
...
int nas_5gs_send_configuration_update_command_plain_text(
        amf_ue_t *amf_ue, gmm_configuration_update_command_param_t *param)
{
    int rv;
    ogs_pkbuf_t *gmmbuf = NULL;
    ...
    ogs_info("[%s] Configuration update command plain text", amf_ue->supi);

    if (amf_ue->t3555.pkbuf) {
        gmmbuf = amf_ue->t3555.pkbuf;
        ogs_expect_or_return_val(gmmbuf, OGS_ERROR);
        gmmbuf = gmm_build_configuration_update_command_plain_text(amf_ue, param); 
...
int nas_5gs_send_configuration_update_command_replay(
        amf_ue_t *amf_ue, gmm_configuration_update_command_param_t *param)
{
    int rv;

    ogs_pkbuf_t *config_update_cmd_replay_buf_backup = NULL;
    config_update_cmd_replay_buf_backup = ogs_pkbuf_copy(config_update_cmd_replay_buf);

    rv = nas_5gs_send_to_downlink_nas_transport(amf_ue, config_update_cmd_replay_buf_backup);
