        ...
        uint16_t max_num_of_istreams, uint16_t max_num_of_ostreams)
{
    ...
    // for debug the event
    ogs_info("number of events in queue %d",ogs_queue_size(ogs_app()->queue));
    ...
    if (sbi_message!=NULL)
        ogs_info("have sbi message");
        // ogs_info("sbi_message is %s",sbi_message->h.service.name);
    if (nas_message!=NULL)
        ogs_info("have nas message");
        // ogs_info("nas_message is %d",nas_message->gmm.h.message_type);
