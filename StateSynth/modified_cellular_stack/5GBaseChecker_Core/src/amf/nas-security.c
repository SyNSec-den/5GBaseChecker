        ...
        amf_ue_t *amf_ue, ogs_nas_5gs_message_t *message)
{
    ...
    */
    if (new_security_context && dlcounter_reset) {
        dlcounter_reset = false;
        amf_ue->dl_count = 0;
    ...
    ogs_info("amf_ue dl count %d",amf_ue->dl_count);
    ...
    ogs_info("ciphered? %d",ciphered);
    ogs_info("selected enc algo:%d",amf_ue->selected_enc_algorithm);
    if (ciphered) {
        /* encrypt NAS message */
    ...
    *(uint8_t *)(new->data) = h.sequence_number;
    message->h.sequence_number=h.sequence_number;
...
ogs_pkbuf_t *nas_5gs_security_encode_null_security(
        amf_ue_t *amf_ue, ogs_nas_5gs_message_t *message)
{
    int integrity_protected = 0;
    int new_security_context = 0;
    ...
    switch (message->h.security_header_type) {
        ...
        case OGS_NAS_SECURITY_HEADER_INTEGRITY_PROTECTED_AND_CIPHTERD_WITH_NEW_INTEGRITY_CONTEXT:
            ...
            break;
        default:
            ogs_nas_5gs_plain_encode(message);
            break;
//            ogs_error("Not implemented(securiry header type:0x%x)",
//                      message->h.security_header_type);
//            return NULL;
    ...
    */
    ...
    if (new_security_context && dlcounter_reset) { //once dlcounter_reset change to false, never enter here.
        amf_ue->dl_count = 0;
        amf_ue->ul_count.i32 = 0;
    }
...
ogs_pkbuf_t *nas_5gs_security_encode_plain_header(
        amf_ue_t *amf_ue, ogs_nas_5gs_message_t *message)
{
    int integrity_protected = 0;
    int new_security_context = 0;
...
ogs_pkbuf_t *nas_5gs_security_encode_smd_replay(
        amf_ue_t *amf_ue, ogs_nas_5gs_message_t *message)
{
    int integrity_protected = 0;
    int new_security_context = 0;
...
ogs_pkbuf_t *nas_5gs_security_encode_replay(
        amf_ue_t *amf_ue, ogs_nas_5gs_message_t *message, int smd_seq, int conf_upd_seq)
{
    int integrity_protected = 0;
    int new_security_context = 0;
    int ciphered = 0;
    ...
    */
    ...
    if (smd_seq!=-1) h.sequence_number=smd_seq;
    if (conf_upd_seq!=-1) h.sequence_number=conf_upd_seq;
    ogs_info("conf_upd_seq 501=%d",conf_upd_seq);

    ogs_info("seq number %d",h.sequence_number);
    ogs_info("amf_ue dl count %d",amf_ue->dl_count);
    ...
    *(uint8_t *)(new->data) = h.sequence_number;
    ...
    /* increase dl_count */
    amf_ue->dl_count = (amf_ue->dl_count + 1) & 0xffffff; /* Use 24bit */
    ogs_info("amf_ue dl count 324 %d",amf_ue->dl_count);
