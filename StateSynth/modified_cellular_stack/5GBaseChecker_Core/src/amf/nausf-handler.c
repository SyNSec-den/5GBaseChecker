...
int amf_nausf_auth_handle_authenticate(  // send auth req in line 116
        amf_ue_t *amf_ue, ogs_sbi_message_t *message)  // has message param
{
    ...
    OpenAPI_list_for_each(UeAuthenticationCtx->_links, node) {
        ...
        if (LinksValueScheme) {
            if(!LinksValueScheme->key) {
                ogs_error("key is null");
            }
    ...
    ogs_info("interrupt_authentication_request nausf 114");
    ogs_assert(OGS_OK ==
...
{
    ogs_info("enter ausf confirmation");
