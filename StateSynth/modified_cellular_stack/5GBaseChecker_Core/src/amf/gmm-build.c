...
#include "gmm-handler.h" // send_message
...
ogs_pkbuf_t *gmm_build_registration_accept(amf_ue_t *amf_ue)
{
    ...
    message.h.security_header_type =
        OGS_NAS_SECURITY_HEADER_INTEGRITY_PROTECTED_AND_CIPHERED;  // protected
...
ogs_pkbuf_t *gmm_build_registration_accept_plain_text(amf_ue_t *amf_ue)
{
    int served_tai_index = 0;
    ogs_pkbuf_t *pkbuf = NULL;
    ...
    memset(&message, 0, sizeof(message));
    message.h.security_header_type =
            OGS_NAS_SECURITY_HEADER_PLAIN_NAS_MESSAGE; // plain_text
    message.h.extended_protocol_discriminator =
            OGS_NAS_EXTENDED_PROTOCOL_DISCRIMINATOR_5GMM;
    ...
    pkbuf = nas_5gs_security_encode_null_security(amf_ue, &message);

    return pkbuf;
}

ogs_pkbuf_t *gmm_build_registration_accept_h4_plain_text(amf_ue_t *amf_ue)
{
    int served_tai_index = 0;
    ogs_pkbuf_t *pkbuf = NULL;
    ...
    message.h.security_header_type = OGS_NAS_SECURITY_HEADER_INTEGRITY_PROTECTED_AND_CIPHTERD_WITH_NEW_INTEGRITY_CONTEXT; // header 4
...
ogs_pkbuf_t *gmm_build_de_registration_request(amf_ue_t *amf_ue)
{
    ...
    dereg_req->de_registration_type.switch_off = 0;
...
ogs_pkbuf_t *gmm_build_de_registration_request_plain_text(amf_ue_t *amf_ue)
{
    ogs_nas_5gs_message_t message;
    ogs_nas_5gs_deregistration_request_to_ue_t *dereg_req =
            &message.gmm.deregistration_request_to_ue;
    ...
    memset(&message, 0, sizeof(message));
    message.h.security_header_type =
            OGS_NAS_SECURITY_HEADER_PLAIN_NAS_MESSAGE; //plaintext here
    message.h.extended_protocol_discriminator =
            OGS_NAS_EXTENDED_PROTOCOL_DISCRIMINATOR_5GMM;
    ...
    return nas_5gs_security_encode_null_security(amf_ue, &message);
}
...
//     }
    ...
    ogs_debug("    Replayed UE SEC[LEN:%d NEA:0x%x NIA:0x%x EEA:0x%x EIA:0x%x",
            ...
            replayed_ue_security_capabilities->eutra_ea,
            replayed_ue_security_capabilities->eutra_ia);
    ogs_warn("    Selected[Integrity:0x%x Encrypt:0x%x]",
    ...
    ogs_pkbuf_t * result_buf=nas_5gs_security_encode(amf_ue, &message);
    smd_seq=message.h.sequence_number;
    return result_buf;
}

ogs_pkbuf_t *gmm_build_security_mode_command_replay(amf_ue_t *amf_ue)
{
    ogs_nas_5gs_message_t message;
    ogs_nas_5gs_security_mode_command_t *security_mode_command =
            &message.gmm.security_mode_command;
    ...
    memset(&message, 0, sizeof(message));
    message.h.security_header_type =
            OGS_NAS_SECURITY_HEADER_INTEGRITY_PROTECTED_AND_NEW_SECURITY_CONTEXT;  // change security header type here
    message.h.extended_protocol_discriminator =
            OGS_NAS_EXTENDED_PROTOCOL_DISCRIMINATOR_5GMM;
    ...
    return nas_5gs_security_encode_replay(amf_ue, &message, smd_seq, -1);
}

ogs_pkbuf_t *gmm_build_security_mode_command_protected(amf_ue_t *amf_ue)
{
...
//     }
    ...
    memset(&message, 0, sizeof(message));
    message.h.security_header_type =
            OGS_NAS_SECURITY_HEADER_INTEGRITY_PROTECTED_AND_CIPHERED;  // change security header type here: ciphered
    message.h.extended_protocol_discriminator =
            OGS_NAS_EXTENDED_PROTOCOL_DISCRIMINATOR_5GMM;
    ...
    ogs_info("int algorithm:%d enc algorithm:%d",amf_ue->selected_int_algorithm,amf_ue->selected_enc_algorithm);
...
ogs_pkbuf_t *gmm_build_security_mode_command_null_security(amf_ue_t *amf_ue)
{
//    if (!check_kamf(amf_ue->kamf)) { //we don't need ctxt check here
//        ogs_info("no amf_ue_kamf");
//        return NULL;
//    }
    ogs_nas_5gs_message_t message;
    ...
    memset(&message, 0, sizeof(message));
    message.h.security_header_type =
            OGS_NAS_SECURITY_HEADER_INTEGRITY_PROTECTED_AND_NEW_SECURITY_CONTEXT;  // 3
    message.h.extended_protocol_discriminator =
            OGS_NAS_EXTENDED_PROTOCOL_DISCRIMINATOR_5GMM;
    ...
    amf_ue->selected_int_algorithm = amf_selected_int_algorithm(amf_ue);
    amf_ue->selected_enc_algorithm = amf_selected_enc_algorithm(amf_ue);
    selected_nas_security_algorithms->type_of_integrity_protection_algorithm = 0;
    selected_nas_security_algorithms->type_of_ciphering_algorithm = 0;
...
ogs_pkbuf_t *gmm_build_security_mode_command_null_security_int(amf_ue_t *amf_ue)
{
//     if (!check_kamf(amf_ue->kamf)) {
//         ogs_info("no amf_ue_kamf");
//         return NULL;
//     }
    ...
    return nas_5gs_security_encode(amf_ue, &message); //this msg will have MAC
}
...
//
//    configuration_update_indication->acknowledgement_requested = 1;
//
...
//    }
    ...
    ogs_pkbuf_t * result_buf=nas_5gs_security_encode(amf_ue, &message);
    guti_indicator = guti_indicator + 1;
    return result_buf;
}

ogs_pkbuf_t *gmm_build_configuration_update_command_plain_text(
        amf_ue_t *amf_ue, gmm_configuration_update_command_param_t *param)
{
    ogs_nas_5gs_message_t message;
    ogs_nas_5gs_configuration_update_command_t *configuration_update_command =
    ...
    ogs_pkbuf_t * result_buf=nas_5gs_security_encode_null_security(amf_ue, &message);

    return result_buf;
}

ogs_pkbuf_t *gmm_build_configuration_update_command_replay(
        amf_ue_t *amf_ue, gmm_configuration_update_command_param_t *param)
{
    ogs_info("enter config update replay");
    ogs_nas_5gs_message_t message;
    ogs_nas_5gs_configuration_update_command_t *configuration_update_command =
            &message.gmm.configuration_update_command;
    ...
    return nas_5gs_security_encode_replay(amf_ue, &message, -1, conf_upd_seq);
}
