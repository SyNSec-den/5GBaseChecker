...
static amf_context_t self;    // global/static amf context
...
// gnb: ngap state
{
    ...
    // assign gnb to event e
    // and init with this event
    memset(&e, 0, sizeof(e));
    e.gnb = gnb;
    ogs_fsm_init(&gnb->sm, ngap_state_initial, ngap_state_final, &e);

    ogs_list_add(&self.gnb_list, gnb);  // add list to gnb
...
void amf_gnb_remove(amf_gnb_t *gnb)
{
    ...
    memset(&e, 0, sizeof(e));
    e.gnb = gnb;   // assign gnb to event
    ogs_fsm_fini(&gnb->sm, &e);   // final
    ...
    ogs_pool_free(&amf_gnb_pool, gnb);  // free this gnb
...
amf_ue_t *amf_ue_add(ran_ue_t *ran_ue)
{
    ...
    ogs_list_add(&self.amf_ue_list, amf_ue);    // add new ue to amf_ue_list
    ogs_info("self addr in context: %p",&self);
...
// ue: gmm state
{
    ...
    memset(&e, 0, sizeof(e));
    e.amf_ue = amf_ue;   // assign ue to event
    ogs_info("amf_event_created");
    ogs_fsm_init(&amf_ue->sm, gmm_state_initial, gmm_state_final, &e);
    // init ue state machine with this event
    // gmm_state_initial, gmm_state_final: function ptr
    // gmm_state_initial -> gmm_state_deregistered -> common register state
}
...
void amf_ue_fsm_fini(amf_ue_t *amf_ue)
{
    ...
    memset(&e, 0, sizeof(e));
    e.amf_ue = amf_ue;   // assign ue to event
    ogs_fsm_fini(&amf_ue->sm, &e); // final state
}
        ...
        ogs_nas_5gs_mobile_identity_t *mobile_identity)
{
    ...
    suci = ogs_nas_5gs_suci_from_mobile_identity(mobile_identity);
    // ogs_assert(suci);  // comment out this
    ...
    if (amf_ue->suci) {
        ogs_hash_set(self.suci_hash, amf_ue->suci, strlen(amf_ue->suci), NULL);
//        ogs_free(amf_ue->suci); // this will cause core crash.
    }
    amf_ue->suci = suci;
...
{
    ogs_assert(supi);
    ogs_warn("amf_ue_set_supi ogs_assert(supi) passed.");
    if (amf_ue->supi) {
        ogs_warn("amf_ue->supi in amf context: %s", amf_ue->supi);
        ogs_hash_set(self.supi_hash, amf_ue->supi, strlen(amf_ue->supi), NULL);
        // amf_ue->supi = "imsi-001011234567895"; //srsue bug?
//        ogs_free(amf_ue->supi); // this will cause core crash. [enable_s1 registration_accept_plain_text auth_request_plain_text id_request_plain_text]. The last id_request_plain_text will be failed(core crashed).
    }
