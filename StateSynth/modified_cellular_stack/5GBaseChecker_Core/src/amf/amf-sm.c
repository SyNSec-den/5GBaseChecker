    ...
    case OGS_EVENT_SBI_CLIENT:
        ...
        CASE(OGS_SBI_SERVICE_NAME_NNRF_NFM)
            ...
            CASE(OGS_SBI_RESOURCE_NAME_NF_INSTANCES)
                ...
                e->h.sbi.message = &sbi_message;   // e->h.sbi.message update here
                // ogs_info("set e->h.sbi.message=[%s]",e->h.sbi.message);
        ...
        CASE(OGS_SBI_SERVICE_NAME_NPCF_AM_POLICY_CONTROL)
            ...
            if (amf_ue) {
                ...
                ogs_info("%s",sbi_message.h.service.name);
                e->amf_ue = amf_ue;
        ...
        CASE(OGS_SBI_SERVICE_NAME_NAUSF_AUTH)  
            ...
            if (amf_ue) {
                ogs_assert(OGS_FSM_STATE(&amf_ue->sm));
                ogs_info("event auth addr: [%p]",event_auth);
                ogs_info("%s",sbi_message.h.service.name);
                e->amf_ue = amf_ue;
                e->h.sbi.message = &sbi_message; 
                event_auth=e;
                ...
                ogs_info("signal in amf-sm 387");
                auth_count++;
                ogs_info("auth count:%d",auth_count);

                if ((strcmp(sbi_message.h.service.name,OGS_SBI_SERVICE_NAME_NAUSF_AUTH)==0)&&(auth_count==1)) {
                    pthread_mutex_lock(&send_auth_cond_mutex);
                    pthread_cond_wait(&send_auth_cond, &send_auth_cond_mutex);
