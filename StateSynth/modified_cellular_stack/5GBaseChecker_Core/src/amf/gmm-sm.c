...
volatile ogs_fsm_t *state;

volatile amf_event_t *event_auth = NULL;
volatile amf_event_t event_test_auth;
volatile OpenAPI_av5g_aka_t *auth_data_ptr = NULL;
volatile OpenAPI_list_t* links_ptr = NULL;
volatile OpenAPI_ue_authentication_ctx_t *UeAuthCtx=NULL;
...
// e->h.sbi.message update every 10 sec ?
...
//                OGS_FSM_TRAN(s, gmm_state_exception);
            ...
            /* in case first msg is auth request
            xact_count = amf_sess_xact_count(amf_ue);
            ...
            *******************************
            ...
            amf_ue->allowed_nssai.s_nssai[0].sst=1;
            amf_ue->allowed_nssai.s_nssai[0].mapped_hplmn_sst=0;
            amf_ue->allowed_nssai.s_nssai[0].sd.v=0xffffff;
            amf_ue->allowed_nssai.s_nssai[0].mapped_hplmn_sd.v=0xffffff;
            amf_ue->allowed_nssai.num_of_s_nssai++;

            if (!AMF_UE_HAVE_SUCI(amf_ue)) {
                ogs_info("interrupt_nas_identity_request");
                CLEAR_AMF_UE_TIMER(amf_ue->t3570);
                ogs_info ("num of request nssai = %d",amf_ue->requested_nssai.num_of_s_nssai);
                /*
                int rv=amf_update_allowed_nssai(amf_ue);
                if (rv==false) {
                    ogs_info("cannot update num of nssai");
                }
                 */
                /*
                ogs_assert(OGS_OK ==
            ...
            else { // for debugging, have suci

                ogs_info("interrupt_nas_identity_request");
                ogs_info ("num of request nssai = %d",amf_ue->requested_nssai.num_of_s_nssai); ogs_info("amf_ue suci: %s",amf_ue->suci);
...
//                OGS_FSM_TRAN(s, gmm_state_exception);
            ...
            // after get id response, send ue info to ausf
            ...
            */
            ...
            OGS_FSM_TRAN(s, &gmm_state_authentication); // break state trans ?
            ogs_info("transferred to auth state");

            // after identity request, FSM change to authentication
            break;
...
//            ogs_info("[%s] Deregistration accept from UE", amf_ue->supi);
//            send_message("deregistration_accept\n");
//            break;
            /*=
...
//                OGS_FSM_TRAN(s, gmm_state_exception);
        ...
        case OGS_NAS_5GS_SECURITY_MODE_COMPLETE:
            ogs_info("get sec mode complete");
            CLEAR_AMF_UE_TIMER(amf_ue->t3560);
            int rv3= gmm_handle_security_mode_complete(
                    amf_ue,&nas_message->gmm.security_mode_complete);
            if (rv3 != OGS_5GMM_CAUSE_REQUEST_ACCEPTED) {
...
//                OGS_FSM_TRAN(s, &gmm_state_exception);
            ...
            /********* important: derive amf_ue->kgnb  *********/
            ogs_info("try to derive amf_ue kgnb");
            ...
            ogs_info("send a initial setup request without registration accept");
            ogs_pkbuf_t* ngapbuf = ngap_ue_build_initial_context_setup_request(amf_ue, NULL);
            int rv5 = nas_5gs_send_to_gnb(amf_ue, ngapbuf);

            int i;
            for (i=0; i<OGS_SHA256_DIGEST_SIZE; i++)
                ogs_info("%d",amf_ue->kgnb[i]);
        ...
        case OGS_NAS_5GS_SECURITY_MODE_REJECT:
            ogs_info("get sec mode rej");
            CLEAR_AMF_UE_TIMER(amf_ue->t3560);
            send_message("nas_sm_reject\n"); // no handle, send here
            return;

        case OGS_NAS_5GS_REGISTRATION_COMPLETE:
            ogs_info("get regis acc");
            send_message("registration_complete\n"); // no handle, send here
            CLEAR_AMF_UE_TIMER(amf_ue->t3550);
            ogs_info("amf_ue suci: %s",amf_ue->suci);
            if (amf_ue->next.m_tmsi) {
                amf_ue_confirm_guti(amf_ue);
            } else {
                ogs_error("[%s] No GUTI allocated", amf_ue->supi);
        ...
        case OGS_NAS_5GS_CONFIGURATION_UPDATE_COMPLETE:
            ogs_info("get configuration update complete %s", amf_ue->supi);
            send_message("config_update_complete\n"); // no handle, send here
            if (amf_ue->next.m_tmsi) {
                amf_ue_confirm_guti(amf_ue);
            } else {
        ...
        case OGS_NAS_5GS_AUTHENTICATION_RESPONSE:
            ogs_debug("get auth response");
            // send_message("auth_response\n"); // no handle, send here
            int rv2= gmm_handle_authentication_response(amf_ue,&nas_message->gmm.authentication_response);
            return;
    ...
    case AMF_EVENT_5GMM_TIMER:
        ogs_info("enter AMF_EVENT_5GMM_TIMER");
        break;
        switch (e->h.timer_id) {
        ...
        case AMF_TIMER_T3555:
            if (amf_ue->t3555.retry_count >=
                ...
                ogs_info("interrupt config upd cmd");
        ...
        case AMF_TIMER_T3570:
            if (amf_ue->t3570.retry_count >=
                ...
                /*
                ogs_info("interrupt_nas_identity_request");
                // printf("interrupt_nas_identity_request");
    ...
    case OGS_EVENT_SBI_CLIENT:
        ...
        CASE(OGS_SBI_SERVICE_NAME_NAUSF_AUTH)  // authentication
            SWITCH(sbi_message->h.resource.component[0])
            CASE(OGS_SBI_RESOURCE_NAME_UE_AUTHENTICATIONS)
                ...
                CASE(OGS_SBI_HTTP_METHOD_POST)
                    // interrupt auth req
                    ogs_info("interrupt auth req");
                    /*
                    rv = amf_nausf_auth_handle_authenticate( // send authentication request
                         amf_ue, sbi_message);    // param: amf_ue, sbi_message
                ...
                CASE(OGS_SBI_HTTP_METHOD_PUT)
                    ...
                    /* don't transfer to sec mode state here
                    else {  // can handle auth response ?
                        OGS_FSM_TRAN(&amf_ue->sm, &gmm_state_security_mode);
                        // amf_ue FSM change to security_mode_command
                        // all amf_ue trans to gmm_state_exception, except this
                    }
                     */
...
// OGS_EVENT_SBI_CLIENT
{
    ...
    case AMF_EVENT_5GMM_MESSAGE:
        ogs_info("enter auth state AMF_EVENT_5GMM_MESSAGE");
        nas_message = e->nas.message;
        ogs_assert(nas_message);
...
//            OGS_FSM_TRAN(s, &gmm_state_exception);
        ...
        case OGS_NAS_5GS_IDENTITY_RESPONSE:
            ogs_info("get id response");
            CLEAR_AMF_UE_TIMER(amf_ue->t3570);
            int rv2= gmm_handle_identity_response(amf_ue,&nas_message->gmm.identity_response);
            return;
...
//                OGS_FSM_TRAN(s, &gmm_state_exception);
        ...
        break;
    case AMF_EVENT_5GMM_TIMER:
        ogs_info("enter auth state AMF_EVENT_5GMM_TIMER");
        break;
        switch (e->h.timer_id) {
        case AMF_TIMER_T3560:
            ogs_info("amf_timer_t3560");
            if (amf_ue->t3560.retry_count >=
                ...
                ogs_info("interrupt_nas_authentication_request");
        ...
        break;
    case OGS_EVENT_SBI_CLIENT:
        ogs_info("enter auth state OGS_EVENT_SBI_CLIENT");
        sbi_response = e->h.sbi.response;
        ogs_assert(sbi_response);
        sbi_message = e->h.sbi.message;
        ogs_assert(sbi_message);
        ...
        DEFAULT
            ...
            // ogs_assert_if_reached(); for debug
        END
        break;
...
// AMF_EVENT_5GMM_TIMER
{
    ...
    switch (e->h.id) {
    case OGS_FSM_ENTRY_SIG:
        ogs_info("interrupt_security_mode_command");
        /*
        CLEAR_AMF_UE_TIMER(amf_ue->t3560);
        ogs_assert(OGS_OK ==
...
//            OGS_FSM_TRAN(s, &gmm_state_exception);
        ...
        case OGS_NAS_5GS_AUTHENTICATION_RESPONSE:
            ogs_info("get auth response");
            // handle
            int rv3= gmm_handle_authentication_response(amf_ue,&nas_message->gmm.authentication_response);
            return;

        case OGS_NAS_5GS_AUTHENTICATION_FAILURE:
            ogs_info("get auth failure");
            send_message("auth_failure\n"); // no handle. need modify
            return;
...
//        ogs_assert(sbi_response);
        ...
        // auth response get confirmation
        CASE(OGS_SBI_SERVICE_NAME_NAUSF_AUTH)  // authentication
            SWITCH(sbi_message->h.resource.component[0])
            CASE(OGS_SBI_RESOURCE_NAME_UE_AUTHENTICATIONS)
...
// AMF_EVENT_5GMM_TIMER
{
    ...
    case AMF_EVENT_5GMM_MESSAGE:
        ...
        case OGS_NAS_5GS_REGISTRATION_COMPLETE:
            ...
            ogs_info("interrupt conf upd cmd");
...
//            OGS_FSM_TRAN(s, &gmm_state_exception);
        ...
        case OGS_NAS_5GS_AUTHENTICATION_RESPONSE:
            ogs_info("get auth response");
            // handle
            int rv4= gmm_handle_authentication_response(amf_ue,&nas_message->gmm.authentication_response);
            return;
...
//                OGS_FSM_TRAN(s, gmm_state_exception);
            ...
            if (!AMF_UE_HAVE_SUCI(amf_ue)) {
                ...
                // copied from last if
                ogs_info("amf ue have SUCI, no identity_request sent?");
